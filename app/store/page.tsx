      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-12">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="flex gap-4 pt-4">
            <button onClick={() => onAddToCart(product)} className="flex-1 py-4 bg-zinc-900 border border-zinc-800 text-white rounded-xl text-sm uppercase font-bold">
              Add To Cart
            </button>
            <button onClick={() => onBuyNow(product)} className="flex-1 py-4 bg-emerald-500 hover:bg-emerald-400 text-black rounded-xl text-sm font-black uppercase">
              Buy It Now
            </button>
          </div>
        </div>
...
export default function StorePage() {
  ...
  const handleBuyNow = (product: Product) => {
    ...
    handleForwardToCheckout(checkoutCart);
  };
  ...
  const filteredProducts = products.filter((p) => {
    ...
    return p.category === viewState && (activeSub === "All Gear" || p.subcategory === activeSub);
  });
      ...
      <div className="bg-zinc-900 border-b border-zinc-800 px-6 py-4 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4">
            ...
            <button onClick={() => setIsCartOpen(true)} className="bg-zinc-950 border border-zinc-800 px-4 py-2 rounded-xl text-xs flex items-center gap-2">
              Cart <span className="bg-emerald-500 text-black px-1.5 rounded-full font-black">{cart.reduce((acc, i) => acc + i.quantity, 0)}</span>
            </button>
          </nav>
        </div>
      </div>
      ...
      {selectedProduct ? (
        ...
        <div className="max-w-6xl mx-auto px-6 py-8 space-y-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            <div onClick={() => { setViewState(padelCategory.name); }} className="lg:col-span-7 relative h-80 rounded-2xl overflow-hidden cursor-pointer bg-zinc-900 border border-zinc-800 p-8 flex flex-col justify-end">
              <img src={padelCategory.img} className="absolute inset-0 w-full h-full object-cover opacity-40" alt="" onError={(e) => { (e.target as HTMLImageElement).src = padelCategory.fallback; }} />
              <h3 className="text-3xl font-black relative z-10">{padelCategory.name} Gear</h3>
            </div>
            <div className="lg:col-span-5 grid grid-cols-2 gap-4">
              {rightGridCategories.map(sport => (
                <div key={sport.name} onClick={() => { setViewState(sport.name); }} className="relative h-36 rounded-2xl overflow-hidden cursor-pointer bg-zinc-900 border border-zinc-800 p-4 flex flex-col justify-end">
                  <img src={sport.img} className="absolute inset-0 w-full h-full object-cover opacity-30" alt="" onError={(e) => { (e.target as HTMLImageElement).src = sport.fallback; }} />
        ...
        /* Subcategory Department Listings */
        <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {filteredProducts.map((p, idx) => (
              <div key={idx} onClick={() => setSelectedProduct(p)} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 cursor-pointer">
          ...
          <div className="w-full max-w-md bg-zinc-900 border-l border-zinc-800 p-6 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-center border-b border-zinc-800 pb-4 mb-4">
                <h3 className="font-bold uppercase tracking-wider">Your Bag</h3>
                ...
                {cart.map((item, idx) => (
                  <div key={idx} className="flex gap-4 p-3 bg-zinc-950 border border-zinc-800 rounded-xl items-center">
                    <div className="flex-grow">
                      ...
                      <p className="text-xs text-emerald-400 font-bold">{item.product.elitePrice}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <button onClick={() => updateCartQty(idx, -1)} className="px-1.5 bg-zinc-900 rounded">-</button>
            ...
            {cart.length > 0 && (
              <div className="pt-4 border-t border-zinc-800 space-y-4">
                <div className="flex justify-between">
                  <span className="text-xs uppercase text-zinc-400 font-bold">Total Amount:</span>
                  <span className="text-xl font-black text-emerald-400">{getCartTotal().toLocaleString()} PKR</span>
                </div>
                <button onClick={() => handleForwardToCheckout(cart)} className="w-full bg-emerald-500 text-black font-black py-4 rounded-xl text-xs uppercase tracking-wider">
